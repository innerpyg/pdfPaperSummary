...
def download_pdf_with_retry(pmc_id:str, outdir:str, headers:Dict[str, str], max_retries:int=3) -> Optional[str]:
    """
    PDF 다운로드를 시도하고 유효성 검사 후 필요시 재시도합니다.
    ...
    try:
        ...
        if pdf_meta:
            ...
            for attempt in range(max_retries):
                try:
                    # 기존 download_pdf 함수의 PDF 다운로드 로직
                    ...
                    if pdf_response.status_code == 200:
                        ...
                        if is_valid_pdf(pdf_content):
                            ...
                            logger.warning(f"유효하지 않은 PDF 파일 (시도 {attempt+1})")
                            # 잠시 대기 후 재시도
                            time.sleep(2)
                    else:
                        logger.warning(f"PDF 다운로드 실패: HTTP 상태 코드 {pdf_response.status_code} (시도 {attempt+1})")
                        time.sleep(2)
                        
                except Exception as e:
                    logger.error(f"PDF 다운로드 중 오류 발생: {str(e)} (시도 {attempt+1})")
                    time.sleep(2)   
...
def download_pdf(pmc_id: str, outdir: str, headers: Dict[str, str]) -> Optional[str]:
    """
    PMC ID에 해당하는 PDF를 다운로드합니다.
    ...
    try:
        ...
        if pdf_meta:
            ...
            if pdf_response.status_code == 200:
                ...
                logger.error(f"PDF 다운로드 실패: HTTP 상태 코드 {pdf_response.status_code}")
        else:
            logger.warning(f"PMC{pmc_id}의 citation_pdf_url을 찾을 수 없습니다.")
...
def main(args: Any) -> None:
    ...
    try:
        ...
        # 이미 작업한 논문 목록 가져오기
        # processed_pmcs = get_processed_pmcs()
        # logger.info(f"이미 작업한 논문 수: {len(processed_pmcs)}개")
        ...
        if best_pmc_id:
            ...
            # 가장 연관성 높은 논문 다운로드
            # pdf_path = download_pdf(best_pmc_id, outdir, headers)
